...
double trace(matrix<double>& c, std::vector<int>& sendcounts, std::vector<int>& displs, MPI_Comm comm) {
    double trace = 0.0;
    int rowc_c = c.nrows();
    int col_c = c.ncols();
    ...
    for (int i = 0; i < rowc_c; ++i) 
    {
        j = i + displs[myrank]/col_c;
        trace += c(i, j); 
    }

    std::cout <<  displs[myrank]/col_c << '\n';
    return trace;
}
...
int main(int argc, char* argv[])
{
    ...
    double trace_loc = trace(c, sendcounts, displs, MPI_COMM_WORLD);
    double trace_all = 0.0;

    MPI_Reduce(&trace_loc, &trace_all, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
